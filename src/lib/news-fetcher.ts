...
async function scrapeArticleContent(url: string): Promise<string> {
  try {
    ...
    const unwantedSelectors = ['script', 'style', 'nav', 'header', 'footer', 'aside', '.ad', '.advertisement', '.comments'];
...
async function generateSummary(content: string): Promise<string | null> {
  ...
  try {
    ...
    console.warn('Summary generation failed, no fallback available');
    return null;
  }
}
 ...
 */
async function fetchFromGNews(query: string, limit: number): Promise<ExternalNewsArticle[]> {
  ...
  try {
    ...
    for (const article of data.articles || []) {
      const content = await scrapeArticleContent(article.url) || article.content || article.description || '';
      const summary = await generateSummary(content);
      
      // Skip articles without proper summaries
      if (!summary) {
        console.log(`⏭️ Skipping article "${article.title}" - no summary generated`);
        continue;
      }
 ...
 */
async function fetchFromNewsAPI(query: string, limit: number): Promise<ExternalNewsArticle[]> {
  ...
  try {
    ...
    const validArticles = (data.articles || []).filter((a: any) => a.content && a.content !== '[Removed]');
    
    for (const article of validArticles) {
      const content = await scrapeArticleContent(article.url) || article.content || '';
      const summary = await generateSummary(content);
...
export async function fetchArticlesWithFallback(
  articlesPerTag: number = 2,
  maxTags: number = 10
): Promise<ExternalNewsArticle[]> {
  console.log(`🔍 Starting news fetch with ${articlesPerTag} articles per tag (max ${maxTags} tags)`);
  console.log(`📊 API Keys available: GNews=${!!GNEWS_API_KEY}, NewsAPI=${!!NEWSAPI_KEY}`);
  ...
  for (const [i, tagName] of tagNames.entries()) {
    try {
      console.log(`🔍 Fetching articles for tag: "${tagName}" (${i + 1}/${tagNames.length})`);
      ...
      if (error instanceof Error && error.message === 'RATE_LIMIT') {
        ...
        try {
          ...
          allArticles.push(...articles);
          console.log(`✅ Tag "${tagName}" (fallback): ${articles.length} articles fetched`);
        } catch (fallbackError) {
          console.error(`❌ Both APIs failed for tag "${tagName}":`, fallbackError);
        }
      } else {
  ...
  console.log(`🎯 Total articles fetched: ${allArticles.length} from ${tagNames.length} tags`);
  return allArticles;
}
...
export async function fetchArticlesForQueries(
  ...
  console.log('⚠️ Warning: fetchArticlesForQueries is deprecated. Use fetchArticlesWithFallback() instead.');
